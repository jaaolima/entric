...
function httpPost($url, $post = null){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($ch, CURLOPT_HEADER, false);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	...
	if ($post){
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, "POST=".json_encode($post));
	}
    $result = curl_exec($ch);
	if($result === false){
        echo 'Curl error: ' . curl_error($ch);
...
function _series() {
    $arr = array();
    $arr["6º Ano - Ensino Fundamental"] = "6º Ano - Ensino Fundamental";
    $arr["7º Ano - Ensino Fundamental"] = "7º Ano - Ensino Fundamental";
    $arr["8º Ano - Ensino Fundamental"] = "8º Ano - Ensino Fundamental";
    $arr["9º Ano - Ensino Fundamental"] = "9º Ano - Ensino Fundamental";
    $arr["1º Ano - Ensino Médio"] = "1º Ano - Ensino Médio";
    $arr["2º Ano - Ensino Médio"] = "2º Ano - Ensino Médio";
    $arr["3º Ano - Ensino Médio"] = "3º Ano - Ensino Médio";
    return $arr;
}
...
function checkLogin($msg = null) {
    ...
    if ((isset($_SESSION['admin_session_id']) == FALSE) or (isset($_SESSION['admin_session_auth']) == FALSE)){
        ...
        $_this = new UtilModel();
        $delete = $_this->delete("sessions", "WHERE session_time < DATE_SUB('".date("Y-m-d H:i:s")."', INTERVAL 6 HOUR)", null);

        $commands['commands'] = $url;
        $commands['session_id'] = chknumber($_SESSION['admin_session_id']);
        $commands['session_key'] = $_SESSION['admin_session_key'];
        $commands['session_auth'] = $_SESSION['admin_session_auth'];
        $commands['session_time'] = date("Y-m-d H:i:s");

        $vdata = array(':session_id'=>$commands['session_id'], ':session_auth'=>$commands['session_auth']);
        $verify = $_this->select_single_to_array("sessions", "*", "WHERE session_id=:session_id AND uid=:session_auth AND session_time > DATE_SUB('".$commands['session_time']."', INTERVAL 6 HOUR)", $vdata);

        if (!empty($verify)) {
            if ($_SERVER['SERVER_NAME'] <> "localhost"){
                openssl_sign($commands['commands'], $signature, $commands['session_key']);
                $commands['signature'] = @bin2hex($signature);
                $signature = hex2bin($commands['signature']);

                if (!empty($commands['commands']) && @openssl_verify($commands['commands'], $signature, $verify['session_key'])){
                    $return = true;
                    $qdata = array(':session_time'=> date("Y-m-d H:i:s"), ':session_id'=> $verify['session_id']);
                    $update = $_this->update('sessions', "WHERE session_id=:session_id", $qdata);

                    $qdata = array(':id_usuario'=> $verify['user_id']);
                    if ($verify['type'] == "paciente"){
                        $type = "paciente";
                        $user = $_this->select_single_to_array("pacientes", "*", "WHERE id_usuario=:id_usuario", $qdata);
                    }
                    else if ($verify['type'] == "prescritor"){
                        $type = "prescritor";
                        $user = $_this->select_single_to_array("prescritores", "*", "WHERE id_usuario=:id_usuario", $qdata);
                    }
                    else if ($verify['type'] == "administrador"){
                        $type = "administrador";
                        $user = $_this->select_single_to_array("admin", "*", "WHERE id_usuario=:id_usuario", $qdata);
                    }
                    else if ($verify['type'] == "patrocinador"){
                        $type = "patrocinador";
                        $user = $_this->select_single_to_array("patrocinadores", "*", "WHERE id_usuario=:id_usuario", $qdata);
                    }
                    $bruker->usuario = $user;
                    $bruker->type = $type;
                ...
                if ($commands['session_key'] == $verify['session_key']){
                    $return = true;
                    $qdata = array(':session_time'=> date("Y-m-d H:i:s"), ':session_id'=> $verify['session_id']);
                    $update = $_this->update('sessions', "WHERE session_id=:session_id", $qdata);
