...
class Database{
	...
	public function select_query2($query = null) {        
    ...
    public function select_to_array_page($tables = null, $fields = '*', $conds = null, $bind = null, $page = 1, $porlinha = 10) {        
        if(! $tb_str = $this->array_to_str($tables)) return FALSE;
        if(! $fld_str =  $this->array_to_str($fields)) $fld_str = '*';
        if($conds) $conds_str = $this->process_conds($conds); else $conds_str ="";
        $limit_str = "LIMIT ".(($page-1)*$porlinha).", ".$porlinha;
        ...
        if ($total_paginas > 1){
            for ($i=1; $i <= $total_paginas; $i++){
                if ($page == $i){
                    if ($page == 1){ $paginacao['A'] = $page; }else{ $paginacao['A'] = $page-1; }
                    if ($page == $total_paginas){ $paginacao['Z'] = $page; }else{ $paginacao['Z'] = $page+1; }
                    $active = 'active';
                }else{
                    $active = $i;  
                }
    ...
    /*
    $bind = array(':campo' => 'campo');
    $dados = $objeto->insert("campo",
                                $bind);
    */
    public function insert($table, $bind, $return_id=TRUE, $ifnotexist = '') {
        if(!is_array($bind)) return FALSE;
        foreach( $bind as $k=>$v){
            $fld[]= str_replace(':', '', $k);       
            $val[]= "".$k."";
        ...
        try {
            $stmt = $this->conn->prepare(
                "INSERT INTO {$table} (".$this->array_to_str($fld).") VALUES (".$this->array_to_str($val).") ".$ifnotexist
            );
            $stmt->execute($bind);
            $last_id = $this->conn->lastInsertId();
            $this->conn->commit();
