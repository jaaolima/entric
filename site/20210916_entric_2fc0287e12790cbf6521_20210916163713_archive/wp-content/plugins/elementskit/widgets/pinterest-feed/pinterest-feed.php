...
class ElementsKit_Widget_Pinterest_Feed extends Widget_Base {
	...
	protected function render_raw() {
		...
		if(empty($settings['pinterest_user_name'])): ?>
            <div><strong>
			<?php esc_html__('Please set a valid username first', 'elementskit')?>
			</strong></div>
		<?php else:
			...
			if($settings['pinterest_feed_type'] == 'board') :
				...
				if(empty($settings['pinterest_board_name'])) : ?>
                    ...
                    <div><strong>
						<?php esc_html__('Please set a valid board name first', 'elementskit')?>
					</strong></div><?php
			...
			if(empty($data['data']['item'])):
				echo esc_html__('0 items fetched!....', 'elementskit');
