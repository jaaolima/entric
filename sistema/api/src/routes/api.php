		...
		if ($result) {
			...
			if ($usuario){
		        $relatorios = $db->select_to_array("relatorios_suplemento r
		                                            ...
		                                            "r.data_criacao, r.codigo, 'Suplemento' as tipo, p.nome, DATE_FORMAT(p.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p.data_criacao, '%d/%m/%Y %h:%i') as data_criacao",
		                                            "where r.id_prescritor_ibranutro = :id
													UNION
													SELECT r2.data_criacao, r2.codigo, 'Dieta Enteral' as tipo, p2.nome, DATE_FORMAT(p2.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p2.data_criacao, '%d/%m/%Y %h:%i') as data_criacao
													...
													UNION
													SELECT r3.data_criacao, r3.codigo, 'Módulos' as tipo, p3.nome, DATE_FORMAT(p3.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p3.data_criacao, '%d/%m/%Y %h:%i') as data_criacao
													FROM relatorios_modulo r3
													inner join pacientes_modulo p3 on r3.id_paciente = p3.id
													where r3.id_prescritor_ibranutro = :id
													UNION
													SELECT r4.data_criacao, r4.codigo, 'Relatório de Alta' as tipo, p4.nome, DATE_FORMAT(p4.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p4.data_criacao, '%d/%m/%Y %h:%i') as data_criacao
													FROM relatorios r4
