			...
			if ($usuario){
				if($login == "ibranutro"){
					$relatorios = $db->select_to_array("relatorios_suplemento r
								...
								"r.id, r.data_criacao as data_criacao_nao_formatada, r.codigo, 'Suplemento' as tipo, p.nome, DATE_FORMAT(p.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p.data_criacao, '%d/%m/%Y %h:%i') as data_criacao",
								"where r.id_prescritor_ibranutro = :id
								UNION
								SELECT r2.id,  r2.data_criacao  as data_criacao_nao_formatada, r2.codigo, 'Dieta Enteral' as tipo, p2.nome, DATE_FORMAT(p2.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p2.data_criacao, '%d/%m/%Y %h:%i') as data_criacao
								...
								UNION
								SELECT r3.id, r3.data_criacao  as data_criacao_nao_formatada, r3.codigo, 'Módulos' as tipo, p3.nome, DATE_FORMAT(p3.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p3.data_criacao, '%d/%m/%Y %h:%i') as data_criacao
								FROM relatorios_modulo r3
								inner join pacientes_modulo p3 on r3.id_paciente = p3.id
								where r3.id_prescritor_ibranutro = :id
								UNION
								SELECT r4.id, r4.data_criacao  as data_criacao_nao_formatada, r4.codigo, 'Relatório de Alta' as tipo, p4.nome, DATE_FORMAT(p4.data_nascimento, '%d/%m/%Y') as data_nascimento, DATE_FORMAT(p4.data_criacao, '%d/%m/%Y %h:%i') as data_criacao
								FROM relatorios r4
