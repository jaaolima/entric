    ...
    $.ajax({
        ...
        success: function( data ){
            if (data.error){
                ...
                // esconder campos em brancos ou mostrar campos com dados =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
                $("#modalfrmproduto .info_nutricionais table > tbody > tr").each(function() {
                    if ($(this).find('input, textarea').val() == ""){
                        $(this).find('input, textarea').closest('tr').hide();
                    }
                    else{
                        $(this).find('input, textarea').closest('tr').show();
                    }
                });
                $("#modalfrmproduto .comp_nutricional table > tbody > tr").each(function() {
                    if ($(this).find('input, textarea').val() == ""){
                        $(this).find('input, textarea').closest('tr').hide();
                    }
                    else{
                ...
                $("#modalfrmproduto .tab-content .form-group").each(function() {
                    if ($(this).find('input, textarea').val() == ""){
                        $(this).find('input, textarea').closest('.form-group').hide();
                    }
                    else{
                        $(this).find('input, textarea').closest('.form-group').show();
                    }
                });
                $("#modalfrmproduto .m_div_indicacao .form-group").each(function() {
                    if ($(this).find('input, textarea').val() == ""){
                        $(this).find('input, textarea').closest('.form-group').hide();
                    }
                    else{
                                ...
                                if (json_dados.length > 1){
                                    if (tab_apresentacao === false){
                                        ...
                                        for (let i = 2; i <=json_dados.length; i++) {
                                            ...
                                            var id = parseInt(navlink)+1;
                                            var p = $.now();
                                            $('#li-nova-apresentacao').before('<li class="nav-item"><span style="position: relative;top: 6px; padding-left: 5px;color: #ee3900;"></span><a href="#apresentacao'+id+'" style="margin-top: -19px;" class="nav-link apresentacao'+id+'" data-toggle="tab">Apresentação '+id+'</a> </li>');
...
$(function(){
    ...
    $('.calculo_fil_todos').change(function () {
        ...
        if (calculo_fil_todos == 'Todos') {
            $('#filtro_apresentacao_oral .filtros_oral .filtros').not(this).prop('checked', true);

        }else{
            $('#filtro_apresentacao_oral .filtros_oral .filtros').not(this).prop('checked', false);
        }
    });
        ...
        $.ajax({
            ...
            success: function( dados ){
                ...
                $.each(dados, function (key, item) {
                    ...
                    var apres_oral = jQuery.parseJSON(item.apres_enteral);
                    $.each(apres_oral, function (apres_oral_key, apres_oral_item) {
                        apresentacao = apresentacao + apres_oral_item +'<br>';
                    });
