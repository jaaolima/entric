...
function fc_cadastrar_paciente(){ 
    ...
    frm.validate({
        ...
        submitHandler: function( form ){
            ...
            $.ajax({
                type: "POST",
                url: "ajax/cadastrar_paciente_modulo", 
                data: formSerialize,
                cache: false,
                dataType: 'json',
                success: function( data ){
...
function fc_atualizar_paciente(){
    ...
    frm.validate({
        ...
        submitHandler: function( form ){
            ...
            $.ajax({
                type: "POST",
                url: "ajax/atualizar_paciente_modulo",
                data: formSerialize,
                cache: false,
                dataType: 'json',
                success: function( data ){
...
function fc_buscar_paciente(){
    ...
    $.ajax({
        type: "POST",
        url: "ajax/buscar_paciente_modulo",
        data: frm,
        cache: false,
        dataType: 'json',
        success: function( data ){
...
function fc_editar_relatorio(id_relatorio){
    $.ajax({
        type: "POST",
        url: "ajax/relatorio_abrir_modulo",
        ...
        success: function( data ){
            if (data.relatorio){
                ...
                $("input[name='cat_modulo[]']:checked").each(function() {
                    let categoriaId = $(this).attr('id'); // Ex: "categoria_modulo_proteina"
                    let categoriaNomeExibicao = $(this).val(); // Ex: "Módulo de Proteína"

                    // IMPORTANTE: Acessar os dados específicos para esta categoria do objeto relatorio
                    let dadosCategoria = relatorio[categoriaId]; // Isso pegará o objeto {"quantidade_tempo": ...}

                    // Gerar IDs únicos para os inputs criados
                    let qtoTempoId = 'qto_tempo_' + categoriaId;
                    let instrucoesUsoId = 'instrucoes_uso_' + categoriaId;
                    let horario1Id = 'horario_1_' + categoriaId; // ID para o primeiro horário

                    // HTML base para a categoria
                    let htmlContent = `
                        <div class="col-sm-6" id="div_${categoriaId}">
                            <div class="row">
                                <div class="col-sm-12 text-center">
                                    <p class="entric_group_destaque mt-0">${categoriaNomeExibicao}</p>
                                </div>
                            </div>
                            <div class="row mt-4">
                                <div class="col-sm-5">Por quanto tempo:</div>
                                <div class="col-sm-7">
                                    <input type="text" required="required" name="qto_tempo[${categoriaId}]" id="${qtoTempoId}" class="form-control">
                                </div>
                            </div>
                            <div class="row mt-4 horario-row">
                                <div class="col-sm-5">Horário(s) (opcional)</div>
                                <div class="col-sm-4">
                                    <input type="text" placeholder="00:00" name="horario[${categoriaId}][]" id="${horario1Id}" class="form-control hora">
                                </div>
                                <button type="button" class="btn btn-secondary ml-2" onclick="novoHorario(this)" name="novo_horario">
                                    <i class="fa fa-plus-circle" aria-hidden="true"></i>
                                </button>
                            </div>
                            <div class="row mt-4">
                                <div class="col-sm-12">Instruções de Uso (opcional):</div>
                                <div class="col-sm-12">
                                    <input type="text" name="instrucoes_uso[${categoriaId}]" id="${instrucoesUsoId}" class="form-control">
                                </div>
                            </div>
                        </div>
                    `;
                    // Adiciona o HTML ao contêiner principal
                    $("#div_modal_fracionamento").append(htmlContent);

                    // AGORA, PREENCHER OS INPUTS RECÉM-CRIADOS SE HOUVER DADOS PARA ESTA CATEGORIA
                    if (dadosCategoria) {
                        // Preencher "Por quanto tempo"
                        if (dadosCategoria.quantidade_tempo) {
                            $("#" + qtoTempoId).val(dadosCategoria.quantidade_tempo);
                        }

                        // Preencher "Instruções de Uso"
                        if (dadosCategoria.instrucoes) {
                            $("#" + instrucoesUsoId).val(dadosCategoria.instrucoes);
                        }

                        // Lidar com horários
                        // Pega o input do primeiro horário que já foi criado
                        let $horarioInput = $("#" + horario1Id);

                        // Se houver dados para o primeiro horário, preenche
                        if (dadosCategoria.horario1) {
                            $horarioInput.val(dadosCategoria.horario1);
                        } else {
                            // Se não houver horario1, mas existir a categoria, pode ser que o campo de horário precise ser limpo
                            $horarioInput.val('');
                        }

                        // Lidar com múltiplos horários (horario2, horario3, etc.)
                        // Adicionar novos campos de horário e preenchê-los
                        let horarioIndex = 2; // Começa pelo horario2
                        while (dadosCategoria['horario' + horarioIndex]) {
                            let horarioValue = dadosCategoria['horario' + horarioIndex];
                            // Chamar a função que adiciona um novo campo de horário, passando o valor
                            // Você precisa adaptar sua função `novoHorario` para aceitar um valor inicial
                            // Ou simular o clique e depois preencher
                            
                            // Exemplo de como você chamaria uma função adaptada:
                            // novoHorarioComValor(document.getElementById(horario1Id), horarioValue, categoriaId); 
                            // ^^^ Isso requer que `novoHorarioComValor` seja criada ou adaptada

                            // Alternativa: Adicionar o HTML do novo horário diretamente aqui
                            let newHorarioInputId = 'horario_' + horarioIndex + '_' + categoriaId;
                            let currentHorarioDiv = $horarioInput.closest('.horario-row'); // Encontra a div pai do horário atual

                            let newHorarioHtml = `
                                <div class="row mt-4 horario-row">
                                    <div class="col-sm-5 offset-sm-5"></div>
                                    <div class="col-sm-4">
                                        <input type="text" placeholder="00:00" name="horario[${categoriaId}][]" id="${newHorarioInputId}" class="form-control hora">
                                    </div>
                                    <button type="button" class="btn btn-danger ml-2" onclick="removerHorario(this)">
                                        <i class="fa fa-minus-circle" aria-hidden="true"></i>
                                    </button>
                                </div>
                            `;
                            $(htmlContent).append(newHorarioHtml);
                            
                            // Anexar o novo HTML *dentro* da div da categoria
                            $("#div_" + categoriaId).append(newHorarioHtml);

                            // Preencher o valor do novo input de horário
                            $("#" + newHorarioInputId).val(horarioValue);
        ...
        if (error_alert == ""){
            ...
            $.ajax({
                ...
                success: function( data ){
                    ...
                    if (data.success){
                        var relatorio_code = $("#relatorio_code").val();
                        window.open("relatorio_modulo/"+relatorio_code, "_blank");
                    }
                    if (data.error){
                        toastr['error'](data.error.message, '', {positionClass: 'toast-top-right' });
                    }
