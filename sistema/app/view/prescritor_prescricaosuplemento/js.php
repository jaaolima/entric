...
function fc_editar_relatorio(id_relatorio){
    $.ajax({
        type: "POST",
        url: "ajax/relatorio_abrir_suplemento",
...
function busca_produto_relatorio(m_calorica, m_proteica){
    ...
    //if ($("input[name='calculo_apres_aberto_po']:checked").length > 0) {
        $.ajax({
            ...
            data: $("#prescritor_calculo").serialize()+"&margem_calorica="+m_calorica+"&margem_proteica="+m_proteica+"&fracionamento_dia="+$("#fracionamento_dia").val()+"&produto_especializado="+$("#calculo_produto_especializado").val(),
            cache: false,
            dataType: 'html',
