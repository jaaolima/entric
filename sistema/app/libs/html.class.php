...
class HTML {
    ...
    function addRow($array, $dbdados = null, $show = null, $_id = null, $row_class = null) {
        $row_id = '';
        if ($show){
            if (strpos($show, 'style') !== false) {
                $row_id = $show;
        ...
        $data = '<div class="row '.$row_class.'" '.$row_id.'>';
            foreach ($array as $field => $dados) {

                if (isset($array[$field]['label']) and ($array[$field]['label'] <> false)){
                    $chklabel = true;
                }
                else{
                    $chklabel = false;
                ...
                $data .=  '<div class="form-group col-sm-'.(isset($array[$field]['col']) ? $array[$field]['col'] : '12').' '.(isset($array[$field]['form-class']) ? $array[$field]['form-class'] : '').'  '.((isset($array[$field]['input-group-append']) and !$chklabel) ? 'input-group' : '').'   '.((isset($array[$field]['input-group-append-group']) and ! !$chklabel) ? 'input-group' : '').'">';
                $valid = '';
                    ...
                    if (isset($array[$field]['type'])) {
                        if ($array[$field]['type'] == 'div'){
                            ...
                            if ($array[$field]['label'] === false){
                                ...
                                if ($array[$field]['label'] <> ""){
                                    $data .=  '<label for="'.$field.'" class="'.$field.'_label">'.$array[$field]['label'].'</label>';
                                }
                            }                            
                        }
                    }else{
                        //if ($array[$field]['label'] === false){
                        if (!isset($array[$field]['label'])){
                        }else{
                            if ($array[$field]['label'] <> ""){
                                $data .=  '<label for="'.$field.'"  class="'.$field.'_label" '.(isset($array[$field]['label-style']) ? 'style="'.$array[$field]['label-style'].'"' : '').'>'.(isset($array[$field]['label']) ? $array[$field]['label'] : '').'</label>';
                            }
                        }                        
                    }
                }
                ...
                if (isset($array[$field]['uploadfiles']) == TRUE){
                    ...
                    $_id = 'id="'.$field.'"';
                    if (isset($array[$field]['id'])){
                        $_id = '';
                    }
 
                    $data .= '<select '.$_id.' name="'.$field.'" '.$valid.' aria-controls="'.$field.'" '.(isset($array[$field]['required']) ? 'required="'.$array[$field]['required'].'"' : '').' '.(isset($array[$field]['readonly']) ? 'readonly="'.$array[$field]['readonly'].'"' : '').' '.(isset($array[$field]['disabled']) ? 'disabled="'.$array[$field]['disabled'].'"' : '').' class="form-control input-sm '.(isset($array[$field]['class']) ? $array[$field]['class'] : '').'">';
                    ...
                    $data .= '<div class="form-radio ">';
                        foreach ($array[$field]['radiobutton'] as $rbfield => $rbdados) {
                            ...
                            $data .= '<input type="radio" id="radio'.$forcoderadio.'" '.(isset($array[$field]['disabled']) ? 'disabled' : '').' class="radio-outlined '.(isset($array[$field]['class']) ? $array[$field]['class'] : '').'" name="'.$field.'" value="'.$rbfield.'" '.$checked.'>  <label for="radio'.$forcoderadio.'" class="radio-green">'.$rbdados.'</label> ';
                        }
                    $data .= '</div>';
                ...
                }else if (isset($array[$field]['radiobuttons']) == TRUE){
                   
                    foreach ($array[$field]['radiobuttons'] as $rbfield => $rbdados) {
                        $checked = "";
                        if (isset($dbdados[$field])) if ($dbdados[$field] == $rbfield) $checked = 'checked';
                        if (isset($array[$field]['checked'])){
                            if ($array[$field]['checked'] == $rbfield) $checked = 'checked';
                ...
                }else if (isset($array[$field]['checkboxs']) == TRUE){
                    foreach ($array[$field]['checkboxs'] as $rbfield => $rbdados) {
                        $checked = "";
                        if (isset($dbdados[$field])) if ($dbdados[$field] == $rbfield) $checked = 'checked';
                        if (isset($array[$field]['checked'])){
                            if ($array[$field]['checked'] == $rbfield) $checked = 'checked';
                        ...
                        $data .= '<div class="custom-control custom-checkbox">';
                            $data .= '<input type="checkbox" id="checkbox'.$forcoderadio.'" '.(isset($array[$field]['disabled']) ? 'disabled' : '').' class="custom-control-input '.(isset($array[$field]['class']) ? $array[$field]['class'] : '').'" name="'.$field.'" value="'.$rbfield.'" '.$checked.'>  <label for="checkbox'.$forcoderadio.'" class="custom-control-label">'.$rbdados.'</label> ';
                        $data .= '</div>';
                    }
                        ...
                        if ($array[$field]['type'] == 'div'){
                            if (isset($array[$field]['content'])){
                                ...
                                if ($fieldpreview == $field){
                                    $label = "";
                                    if (isset($array[$field]['label'])) $label = '<label '.(isset($array[$field]['class']) ? 'class="'.$array[$field]['class'].'"' : '').'>'.$array[$field]['label'].'</label>';
                    ...
                    if ($diver == 0){
                        ...
                        if (isset($array[$field]['value'])){
                            if (isset($array[$field]['type']) and ($array[$field]['type'] == "email")){
                                ...
                                if (isset($array[$field]['uppercase']) and ($array[$field]['uppercase'] == "nao"))
                                    $valued = 'value="'.$array[$field]['value'].'"';
                                else
                       ...
                       $class_control = "form-control";
                        ...
                        if (($chklabel) and (isset($array[$field]['input-group-prepend']) or isset($array[$field]['input-group-prepend-select']) or isset($array[$field]['input-group-append']) or isset($array[$field]['input-group-append-group']))) {
                        ...
                        if (isset($array[$field]['append'])){
                            $data .= $array[$field]['append'];
                        }
    ...
    function addCol($array) {
        ...
        foreach ($array as $field => $dados) {
            if ($cont == 0){
                $data = '<div class="row '.(isset($array[$field]["form-class"]) ? $array[$field]["form-class"] : '').'">';
                $cont = $cont+1;
            }
            $data .= '<div class="form-group '.(isset($array[$field]["class"]) ? $array[$field]["class"] : '').' col-sm-'.$array[$field]["col"].'">';
    ...
    function addHr($texto = null) {
        $data = '<div class="hr">'.$texto.'</div>';
        return $data;
    }
 
    function addHrSeparador($texto = null, $class=null) {
        $data = '<div class="separator '.$class.'">'.$texto.'</div>';
        return $data;
    }
