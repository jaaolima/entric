...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
			...
			'script_url' => BASE_PATH."/ajax/uploads/",
            ...
            'delete_type' => 'DELETE',
            'access_control_allow_origin' => BASE_PATH,
    ...
    protected function set_additional_file_properties($file) {
		...
		$retorno = explode("/",$dir);
		if (isset($retorno[1])) $retorno = $retorno[1]; else $retorno = "";
        ...
        if ($upload) {
            if (is_array($upload['tmp_name'])) {
                ...
                foreach ($upload['tmp_name'] as $index => $value) {
                    $path_ = ($file_name ? $file_name : $upload['name'][$index]);
                    $ext_ = pathinfo($path_, PATHINFO_EXTENSION);
                    $files[] = $this->handle_file_upload(
                        $upload['tmp_name'][$index],
                        $this->upuninumber().".".$ext_, //$file_name ? $file_name : $upload['name'][$index],
				...
				$path_ = $file_name ? $file_name : (isset($upload['name']) ? $upload['name'] : null);
				$ext_ = pathinfo($path_, PATHINFO_EXTENSION);
                $files[] = $this->handle_file_upload(
                    isset($upload['tmp_name']) ? $upload['tmp_name'] : null,
                    $this->upuninumber().".".$ext_, //$file_name ? $file_name : (isset($upload['name']) ? $upload['name'] : null),
    ...
    public function delete($print_response = true) {
		...
		if ($retorno){
			...
			foreach ($file_names as $file_name) {
				$file_path = $this->get_upload_path($file_name);
                print_r($retorno);
				$filethumb_path = $file_path."thumbnail/".$retorno;
				$file_path = $file_path.$retorno;
				$success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
				@unlink($filethumb_path);
				
				$filedocs_path = str_replace("thumbnail/","", $filethumb_path);
				@unlink($filedocs_path);
