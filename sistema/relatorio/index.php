...
if ($url == "")
	Redirect(BASE_PATH);
$relatorio = $db->select_single_to_array("relatorios", "*, DATE_FORMAT(data_criacao, '%d/%m/%Y %H:%i') as data_criacao", "WHERE id=:id", array(":id" => $url));
if (!$relatorio)
	Redirect(BASE_PATH);
if ($relatorio['codigo'] != "")
	$_GET['imprimir'] = true;
if (($p_header) or ($p_produtos) or ($p_footer)) {
	if ($relatorio['codigo'] == "") {
		die();
	}
}
...
if (trim($relatorio['higienizacao']) == "")
	$relatorio['higienizacao'] = $config['higienizacao'];
if (trim($relatorio['cuidados']) == "")
	$relatorio['cuidados'] = $config['cuidados'];
if (trim($relatorio['preparo']) == "")
...
<head>
	...
	<style>
		...
		.marca-dagua {
			...
			/* Cor semi-transparente (RGBA é ótimo para isso) */
...
<body class="document">
	...
	if (((!$p_produtos) and (!$p_footer)) or ($p_header)) {
		?>
		<div class="page <?php if ($relatorio['rel_logo'] <> "")
			echo "logo_efeito"; ?>" style="position:relative;">
			<?php if ($usuario['login'] == 'ibranutro'): ?>
			...
			<?php endif; ?>
			<p class="text-center <?php if ($usuario['login'] != 'ibranutro'): ?>linha<?php endif; ?> titulo">RELATÓRIO
				NUTRICIONAL DE ALTA</p>


			<?php if ($relatorio['rel_identificacao'] <> "") { ?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?>
					IDENTIFICAÇÃO DO PACIENTE</p>
				<p><strong>Paciente:</strong> <?php echo ucwords($paciente['nome']); ?></p>
				<?php if ($paciente['mae_possui'] == 0) {
					echo "<p><strong>Nome da Mãe:</strong> " . ucwords($paciente['mae']) . "</p>";
				} ?>
			...
			<?php if (trim($relatorio['historia']) <> "") { ?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> HISTÓRIA
					CLÍNICA</p>
			...
			<?php if ($relatorio['rel_avaliacao'] <> "") { ?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> AVALIAÇÃO
					NUTRICIONAL</p>
				...
				?>
				<table width="100%" margin="0" padding="0" border="0" cellspacing="0" cellpadding="0"
						...
						for ($i = 0; $i < count($array_pesos); $i++) {
							...
							<tr>
								<td width="40%" height="30px">
									...
									if ($i == 0) {
										?>
										<p><strong><?php if ($relatorio['altura'] <> "") {
											echo $relatorio['altura'];
										} else {
											echo "Altura";
										} ?>:</strong>
											<?php if ($relatorio['altura'] <> "") {
												echo $relatorio['altura_valor'] . " m";
											} ?>
										</p>
										<?php
									}
						...
						if ($circunferencias) {
							...
							foreach ($circunferencias as $chave => $valores) {
								if ($valores <> "") {
									...
									<tr>
										<td width="40%" height="30px">
											<p><strong><?php if (isset($circunferencias[$chave]))
												echo $circunferencias[$chave]; ?></strong> </p>
										</td>
										<td width="60%">
											<p>
												<?php if (isset($circunferencias_valor[$i]))
													echo $circunferencias_valor[$i] . " cm"; ?>
												<?php if (isset($circunferencia_lado[$chave]))
													echo "&nbsp;&nbsp;&nbsp;" . $circunferencia_lado[$chave]; ?>
											</p>
										</td>
									</tr>
									<?php
						...
						if ($dobras) {
							...
							foreach ($dobras as $chave => $valores) {
								if ($valores <> "") {
									...
									<tr>
										<td width="40%" height="30px">
											<p><strong><?php if (isset($dobras[$chave]))
												echo $dobras[$chave]; ?></strong> </p>
										</td>
										<td width="60%">
											<p>
												<?php if (isset($dobras_valor[$i]))
													echo $dobras_valor[$i] . " cm"; ?>
												<?php if (isset($dobras_lado[$chave]))
													echo "&nbsp;&nbsp;&nbsp;" . $dobras_lado[$chave]; ?>
											</p>
										</td>
									</tr>
									<?php
				...
				<?php if (strlen($relatorio['exames_nutricionais_complementares']) > 3) { ?>
					<p><strong>Exames Nutricionais Complementares:</strong>
						<?php echo $relatorio['exames_nutricionais_complementares']; ?></p>
				<?php } ?>
			...
			<?php if ($relatorio['rel_necessidades'] <> "") { ?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> NECESSIDADES
					NUTRICIONAIS</p>
			...
			if (((!$p_produtos) and (!$p_footer)) or ($p_header)) {
				?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> CONDUTA</p>
				<p>
					...
					<p style='margin:0px;'>Utilizar <?php echo $relatorio['fra_fracionamento_dia']; ?>
						<?php if ($relatorio['fra_fracionamento_dia'] == '1')
							...
							echo "vezes"; ?> ao
						dia<?php if ($relatorio['fra_qto_tempo'] != "")
				...
				?>
				<span>fornecendo de <?php echo $margem_calorica_a; ?> a <?php echo $margem_calorica_b; ?> calorias/dia,
					<?php echo $margem_proteica_a; ?> a <?php echo $margem_proteica_b; ?> gramas de proteína/dia, conforme
					sugestões de produtos abaixo.</span>
				</p>
			...
			<?php if ($relatorio['rel_prescricao'] <> "") { ?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> PRESCRIÇÃO
					NUTRICIONAL ESPECIALIZADA - Escolha uma das opções</p>
				...
				if ($relatorio['tipo_produto'] == "Enteral") {
					...
					if (($relatorio['calculo_apres_aberto_po'] == 1) and (count($_produtos_nomes) > 0)) {
						...
						<table width="100%" margin="0" padding="1" border="1" cellspacing="0" cellpadding="1"
							class="tabela_produtos tabela_p1">
							<thead>
								<tr>
						...
						<span class="modo_uso">
							...
							?>
							<strong>Modo de Uso:</strong> Diluir a quantidade por horário (gramas ou medida) em metade do volume e, após
							misturar bem, completar com água até chegar ao volume total por horário. Instalar dieta
							<?php echo $relatorio['fra_fracionamento_dia']; ?> vezes ao dia, <?php echo $horarios; ?>. Correr cada dieta
							em <?php echo $relatorio['fra_qtas_horas']; ?> horas com oferta de água extra de
							<?php echo $relatorio['fra_volume_horario']; ?> ml por horário, <?php echo $horarios_hidra; ?>.
							<?php
					...
					if (($relatorio['calculo_apres_aberto_liquido'] == 1) and (count($_produtos_nomes) > 0)) {
							...
							if ($relatorio['dieta_produto_dc'] <> "") {
								...
								foreach ($dieta_produto_dc as &$value) {
									...
									if ($produto[6] == "aberto_liquido") {
										...
										if ($produto_cad) {
											$fibra = moeda2float($produto_cad['fibras']);
										} else {
											$fibra = 0;
										}
						...
						<span class="modo_uso">
							...
							?>
							<strong>Modo de Uso:</strong> Fracionar a dieta de acordo com o volume por horário. Instalar a dieta
							<?php echo $relatorio['fra_fracionamento_dia']; ?> vezes ao dia, <?php echo $horarios; ?>. Correr cada dieta
							em <?php echo $relatorio['fra_qtas_horas']; ?> horas com oferta de água extra de
							<?php echo $relatorio['fra_volume_horario']; ?> ml por horário, <?php echo $horarios_hidra; ?>.
							<?php
					...
					if (($relatorio['calculo_apres_fechado'] == 1) and (count($_produtos_nomes) > 0)) {
							...
							if ($relatorio['dieta_produto_dc'] <> "") {
								...
								foreach ($dieta_produto_dc as &$value) {
									...
									if ($produto[6] == "fechado") {
										...
										if (($qtd_horas > 0) and ($volume_final > 0))
											$velocidade = ($volume_final / ($qtd_horas / 60));
										else
											$velocidade = 0;
										$dados_ordem[$produto_cad['fabricante'] . "___" . $produto[1] . "___" . $produto[0]][$cont_dados][] = round_up($velocidade) . " ml/hora";
						...
						<span class="modo_uso">
							...
							?>
							<strong>Modo de Uso:</strong> Instalar dieta às <?php echo $relatorio['fra_h_i_dieta']; ?>. Após o término
							da
							primeira dieta, instalar a próxima (caso haja mais de uma dieta). Correr a dieta em
							<?php echo $relatorio['fra_h_inf_dieta']; ?> h. Com oferta de água extra de
							<?php echo $relatorio['fra_volume_horario']; ?> ml por horário, <?php echo $horarios_hidra; ?>.
							<?php
							if (trim($relatorio['fra_info_complementares']) <> "") {
								echo $relatorio['fra_info_complementares'];
					...
					if (($relatorio['calculo_apres_po'] == 1)) {
						if ($relatorio['dieta_produto_dc'] <> "") {
							...
							if ($dados_ordem != []) {
								...
								<table width="100%" margin="0" padding="1" border="1" style="margin-top: 8px;"
									class="tabela_produtos tabela_p1">
									<thead>
										<tr>
											<th rowspan="2">Produto</th>
			...
			<?php if ((trim($relatorio['rel_observacoes']) <> "")) { ?>
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> OBSERVAÇÕES
				</p>
	...
	?>
		...
		if ($p_footer) {
			...
			<div class="page <?php if ($relatorio['rel_logo'] <> "")
				echo "logo_efeito"; ?>" style="position:relative;">
				<?php if ($usuario['login'] == 'ibranutro'): ?>
					<img class="background" style="position:absolute;left:2cm;width:150px;" src="imagem/logo_ibranutro.png"
						alt="">
				<?php else: ?>
					<img class="background" style="position:absolute;left:2cm;width:150px;" src="imagem/logo.png" alt="">
					<img class="background" style="position:absolute;bottom:1cm;right:2px;" src="imagem/efeito.png" alt="">
		...
		?>
			...
			if (((!$p_produtos) and (!$p_header)) or ($p_footer)) {
				...
				<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
							width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?>
					ORIENTAÇÕES DE PREPARO / MANIPULAÇÃO</p>
				...
				<p>Acompanhe o peso semanalmente e anote ingestão/hidratação em um diário simples para consultas
					...
					Para acesso aos vídeos de orientação de alta, acesse o site: <a href="https://entric.com.br"
						target="_blank">www.entric.com.br</a> ou o QR Code
					disponível nesta orientação.</p>

				<?php if ($usuario['login'] != 'ibranutro'): ?>
					<img src="imagem/simbolo.png" width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" />
					CONTATOS DO PRESCRITOR</p>
					<p>
						<?php
						...
						if ($prescritor) {
							...
							if ($prescritor['profissional'] == "Nutricionista") {
								...
								if (($prescritor['regiao_crn'] <> "") and ($prescritor['numero_crn']))
									echo $prescritor['regiao_crn'] . " - " . $prescritor['numero_crn'];
							} else {
								echo '<strong>' . $prescritor['nome'] . "</strong><br>";
								if (($prescritor['regiao_crn'] <> "") and ($prescritor['numero_crn']))
									echo $prescritor['regiao_crm'] . " - " . $prescritor['numero_crm'];
							}


							if (trim($prescritor['cidade']) <> "")
								echo "<br>" . $prescritor['cidade'] . " " . $prescritor['uf'];
				...
				if ($relatorio['rel_distribuidores'] <> "") {
							...
							<div style="width:50%;padding-left: 10px;padding-right: 10px;">
								...
								<table width="100%" cellspacing="0" cellpadding="0">
									<tbody>
											...
											if ($danone) {
												...
												for ($i = 0; $i < count($danone); $i++) {
													if ($danone[$i]['id'] == '1') {
														...
														if (trim($danone[$i]['endereco']) <> "")
															echo "<br>" . $danone[$i]['endereco'];
														if (trim($danone[$i]['telefone']) <> "")
															echo "<br>" . $danone[$i]['telefone'];
														if (trim($danone[$i]['whatsapp']) <> "")
															echo "<br>" . $danone[$i]['whatsapp'];
														if (trim($danone[$i]['site']) <> "")
															echo "<br>" . $danone[$i]['site'];
														if (trim($danone[$i]['cupom']) <> "")
															echo "<br>Cupom: " . $danone[$i]['cupom'];
														echo "</p>
													</div>";
														echo "";
										...
										<tr>
											<td
												style="width:  100%; border-left: 0px solid #8fcfe5; text-align: center;display:flex;font-size:11px;padding-top:10px;display:flex;flex-wrap:wrap;justify-content:space-around;">
												<?php
					...
					<?php endif; ?>
				<?php } ?>
				<p style="margin-top:10px; font-size:10px; margin-bottom:70px;">Em respeito à sua autonomia e conforme o
					Código de Ética, a lista acima é meramente sugestiva para sua comodidade. A aquisição dos itens
					prescritos é de livre escolha e pode ser realizada em qualquer estabelecimento de sua confiança.</p>
				...
				<?php if ($usuario['login'] == "ibranutro"): ?>
					<div class="print-footer">
						...
						<?php endif; ?>
						<div
							style="display:flex;background-color:darkgray;padding:12px;justify-content:space-between;color:white;">
							<div>
								<p>www.ibranutro.com.br</p>
							</div>
							<div style="margin-left:200px;">
								<p>SHCS CR 515 BI C Entrada 42 salas: 104, 107 e 109 - ENTRADA PELA Via W2 Sul - Asa Sul,
									Brasília - DF, 70381-530</p>
							</div>
						</div>
					</div>
				<?php endif; ?>
				...
				if ($relatorio['codigo'] <> "") {
					?>
					<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img src="imagem/simbolo.png"
								width="18px" border="0" style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> FINAL DO
						RELATÓRIO</p>
					<p>
				...
				<?php if ($relatorio['rel_calculo'] <> "") { ?>
					...
					if ((!$p_header) and (!$p_footer)) {
						...
						if (($relatorio['calculo_apres_aberto_po'] == 1) and (count($_produtos_nomes) > 0)) {
							...
							?>
							<div class="page_land <?php if ($relatorio['rel_logo'] <> "")
								echo "logo_efeito"; ?>" style="page-break-before: always;position:relative;">
								<?php if ($usuario['login'] == 'ibranutro'): ?>
									<img class="background" style="position:absolute;left:2cm;width:150px;" src="imagem/logo_ibranutro.png"
										alt="">
								<?php else: ?>
								...
								<?php if (!isset($_GET['imprimir'])): ?>
									<div class="marca-dagua">RASCUNHO</div>
								<?php endif; ?>
								<p class="text-left subtitutlo"><?php if ($usuario['login'] != 'ibranutro'): ?><img
											src="imagem/simbolo.png" width="18px" border="0"
											style="vertical-align:bottom; margin-right: 5px;" /><?php endif; ?> INFORMAÇÕES NUTRICIONAIS
									COMPLEMENTARES</p>
								...
								<table width="100%" margin="0" padding="1" border="1" cellspacing="0" cellpadding="1"
									style="margin-top: 0.5cm;" class="tabela_produtos tabela_p1">
									<thead>
										<tr>
											<th rowspan="2">Produto</th>
						...
						if (($relatorio['calculo_apres_aberto_liquido'] == 1) and (count($_produtos_nomes) > 0)) {
							...
							<div class="page_land <?php if ($relatorio['rel_logo'] <> "")
								...
								<p>
									<strong>SISTEMA ABERTO (LÍQUIDO)</strong>
								<table width="100%" margin="0" padding="1" border="1" cellspacing="0" cellpadding="1"
									style="margin-top: 0.5cm;" class="tabela_produtos">
									<?php
									if ($relatorio['dieta_produto_dc'] <> "") {
										?>
										<tr>
